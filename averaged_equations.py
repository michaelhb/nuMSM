...
def coefficient_matrix(z, rt, mp, suscT, smdata):
    ...
    return np.array([[-(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[0]) * susc[0, 0]) / 6,
                            ...
                            -(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[0]) * susc[0, 2]) / 6, 0, 0, 0,
                            4 * np.imag(GBt_nu_a[0, 0, 1]), -np.real(GBt_nu_a[0, 0, 0]), -np.real(GBt_nu_a[0, 1, 1]),
                            -2 * np.real(GBt_nu_a[0, 0, 1]), 0],
                           [-(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[1]) * susc[1, 0]) / 6,
                            -(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[1]) * susc[1, 1]) / 6,
                            -(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[1]) * susc[1, 2]) / 6, 0, 0, 0,
                            4 * np.imag(GBt_nu_a[1, 0, 1]), -np.real(GBt_nu_a[1, 0, 0]), -np.real(GBt_nu_a[1, 1, 1]),
                            -2 * np.real(GBt_nu_a[1, 0, 1]), 0],
                           [-(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[2]) * susc[2, 0]) / 6,
                            -(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[2]) * susc[2, 1]) / 6,
                            -(np.power(mp.M * np.exp(-z), 2) * np.real(GB_nu_a[2]) * susc[2, 2]) / 6, 0, 0, 0,
                            4 * np.imag(GBt_nu_a[2, 0, 1]), -np.real(GBt_nu_a[2, 0, 0]), -np.real(GBt_nu_a[2, 1, 1]),
                            -2 * np.real(GBt_nu_a[2, 0, 1]), 0],
                           ...
                           [0, 0, 0, -np.real(GB_N[0, 1]) / 2, -np.real(GB_N[0, 1]) / 2,
                            ...
                            -np.imag(HB_N[0, 1]) / 2, np.imag(HB_N[0, 1]) / 2, 0, 0], [-(
